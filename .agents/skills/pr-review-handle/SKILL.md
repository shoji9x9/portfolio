...
license: MIT
metadata:
    github-path: skills/pr-review-handle
    github-ref: refs/tags/v1.34.0
    github-repo: https://github.com/shoji9x9/skills
    github-tree-sha: 7aa3abc67cd3fec791cb08af8aaff199680b0f90
...
- `--push` 未指定: 全スレッドの返信・解決まで行い、ファイル修正があっても commit / push はしない。ファイル修正があれば変更ファイル一覧と「commit が必要なこと」「`--push` で再実行できること」を通知して止まる。修正がなく返信・解決だけで終わる場合は、後述「レビュー対応後の再レビュー依頼」の確認まで行う（ただし `review_tool: none` では再レビュー依頼をしないため、この確認は省略して完了する）
- `--push`: 上記に加え、関連ファイルだけを stage し論理単位で commit、push する。完了条件（後述「レビュー対応後の再レビュー依頼」。`review_tool: none` では再レビュー依頼の確認自体が不要）を満たしてから締める
...
5. **レビュー本文の指摘を取得する**（後述「レビュー本文に埋め込まれた指摘」）。指摘はスレッドだけでなくレビュー本文にも入るため、
   スレッド 0 件でも本文を読む。本文の指摘は返信先のスレッドが無いのでトップレベルの PR コメントで返す
6. 各未解決スレッドについて、以下を 1 件ずつ行う:
   ...
   4. 返信した後にスレッドを**解決**する
7. 全スレッドとレビュー本文の指摘を処理したら、モードに応じて締める（各モードの挙動は「使い方」を参照）
...
`reviewThreads` にも `comments.totalCount` にも現れない（実測: Copilot は自信度の低い指摘を本文の `<details>` 内へ
`### Comments suppressed due to low confidence (N)` として格納し、ヘッダには `Comments generated: 0 new` と書く。
本文冒頭には `### 🟡 Not ready to approve` のような総合判定も入る）。**未解決スレッド 0 件・生成コメント 0 件でも
未対応の指摘が残り得る**。
...
- 本文の指摘には**返信先のスレッドが無く `resolve` もできない**。トップレベルの PR コメントで、対象箇所・判断・根拠・実測を明記して返す
  （修正した場合も見送った場合も残す）。対応の記録が本文側に残らないため、そのコメントが唯一の証跡になる。
- 総合判定（`Not ready to approve` 等）が残る場合は、どの指摘に基づくか・修正したか見送ったかを報告に明記する。
