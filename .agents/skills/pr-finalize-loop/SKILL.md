...
license: MIT
metadata:
    github-path: skills/pr-finalize-loop
    github-ref: refs/tags/v1.34.0
    github-repo: https://github.com/shoji9x9/skills
    github-tree-sha: d4aa4b5b9fe0a50accd1ecf3a316289eca268789
...
1. **状態取得**（この段階ではレビュー依頼をしない。待機の間隔・上限は後述「ポーリングと待機」に従う）
   ...
   - **レビュー本文の指摘の取得**: 指摘はスレッドだけでなく**レビュー本文**にも入る（後述「レビュー本文に埋め込まれた指摘」）。
     スレッド 0 件・`comments.totalCount` 0 でも本文に未対応の指摘があり得るため、HEAD への非著者レビューの `body` を必ず読む
2. **収束判定（依頼より先に評価する）**: CI が全成功 **かつ** 未解決スレッドが無い **かつ** **レビュー本文に未対応の指摘が無い** **かつ** 現在の HEAD がレビュー済み **かつ** レビュー内容との整合検証を通過 → **完了**。サマリーを報告して終了する。**既に CI・レビューが揃った PR はこの時点でレビュー依頼もせず無害に終わる**。
...
4. **レビュー指摘の解消**（1 件ずつ。スレッドと**レビュー本文の指摘の両方**を対象にする）
   ...
   4. **レビュー本文の指摘には返信先のスレッドが無い**（`resolve` もできない）。トップレベルの PR コメントで、対象箇所・判断・根拠・実測を明記して返す。修正した場合も見送った場合も残す
   5. **人間判断を要する場合**は、ここで修正も解決もせずユーザーに判断を仰ぐ。判断を反映して返信・解決したうえでループを続ける
...
いずれの場合も、終了時に **CI 状態・残った未解決スレッド・レビュー本文に残った指摘や総合判定・反復回数・停止理由**を要約して報告する。未解決スレッドを残して終わる場合でも、**虚偽の解決（中身に対応せず resolve）はしない**。

- **完了**: CI が全成功し、未解決スレッドが無く、**レビュー本文に未対応の指摘が無く**、HEAD がレビュー済み（またはレビュー出現待ちが上限に達した）。`review_tool: none` では HEAD レビュー済みを問わず CI 全成功・未解決スレッド無し・本文の指摘対応済みで完了
- **最大反復到達**: `--max-iterations`（既定 5）に達した。残っている CI 失敗・未解決スレッド・レビュー本文の指摘を明示する
...
**指摘が載る場所はスレッドだけではない。** レビュー本文（`reviews[].body`）に指摘そのものが入ることがあり、この経路は
`reviewThreads` にも `comments.totalCount` にも現れない。**未解決スレッド 0 件・生成コメント 0 件でも未対応の指摘が残り得る**
（実測: Copilot は自信度の低い指摘を本文の `<details>` 内へ `### Comments suppressed due to low confidence (N)` として格納し、
ヘッダには `Comments generated: 0 new` と書く。本文冒頭には `### 🟡 Not ready to approve` のような総合判定も入る）。
...
- 本文の指摘には**返信先のスレッドが無く `resolve` もできない**ため、トップレベルの PR コメントで返す（手順 4-4）。
  対応の記録が本文側に残らないので、レビュー本文の指摘を潰したかは**自分のコメントを唯一の証跡**として扱う。
- 総合判定（`Not ready to approve` 等）が残ったまま完了する場合は、根拠（どの指摘に基づくか・修正したか見送ったか）を報告に明記する。
  必須チェックに影響しない判定であってもユーザーがマージ可否を判断する材料になる。
