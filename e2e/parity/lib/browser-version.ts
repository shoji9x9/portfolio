...
//
// **Issue #31 以降、新側の比較相手は現行側ではない。** 移行が収束し現行側の採取物を凍結したため、
// 新側は「前回の baseline-new」と比べる。したがって `side: "new"` の手当ては新側で閉じる
// （記録値の更新 → 新側ベースラインの採り直し）。`side: "current"` の分岐は移行期の手当てとして残す。
//
...
function remedy(side: CaptureSide, metadataPath: string): string {
  ...
  // Issue #31 以降、比較の相手は現行側ではなく「前回採った新側ベースライン」である
  // （移行は収束し、現行側の採取物は証跡として凍結した）。そのため手当ても新側で閉じる。
  return (
    "比較の基準になる新側ベースライン（baseline-new）は記録値のブラウザーで採られているため、\n" +
    "別のブラウザーで採ると実装差ではない差分が出る。意図したブラウザー更新なら、\n" +
    `  1. ${metadataPath} の capture_conditions.browser を実行中の値へ更新し、\n` +
    "  2. 新側ベースラインを採り直す（この採取をもう一度実行する）。\n" +
    "意図しない差なら、実行環境の Playwright を記録値へ揃えてから採取し直す。"
  );
}
