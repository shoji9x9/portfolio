...
- **ファイル単位でもまだ粗い。同一ファイル内でキーごとに役割が分かれる。** 1 つのキーを証跡と判断しても、
  その判断をファイル全体へ広げない（例: `.replace/parity/<slug>/metadata.json` は `dataset_version` が
  現側採取時点の証跡である一方、`traits.elements` と `capture_conditions` は新側採取が実行時に読む入力で、
  内容を変えたら追従させないと採取が落ちる）。
...
この分類は方針を決める場面だけのものではない。**正本データを変える通常の変更でも、その値から機械的に
導出される記録**（件数・一覧・fingerprint・ハッシュ）を grep で洗う。凍結の判断は「そのとき更新不要
だった記録」であって、**更新不要なファイルの永続的な名簿ではない**。分類を一度書いたら、それを次の
変更でそのまま再利用せず、変えた値の導出先をその都度たどる。
